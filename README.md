...
这是西北工业大学计算机学院2022级编译原理实验课程项目源代码，实验要求完成一个 C 语言子集的编译器，前端采用 Antlr4，保留了 flexbison 和递归下降分析的接口（但未完成），中间 IR 采用自定义的 Dragon IR，类似于 LLVM IR，后端采用 Arm32。

## 0.2 给学弟学妹的话

平心而论，实验难度中等偏难，但不至于做不出来，可能花的时间会多一点。计算机学院在大二应该开设过java的面向对象程序设计，如果你的对面向对象了解一些的话，这个项目的C++代码还是很快能上手的。这个实验做出来的人貌似不多，希望大家可以的话还是自己静下心来做一做，对自己有很大的提升，
并且这个实验框架曾雷杰老师也是花了很大心思的，做下来还是比较有意思。计院虽然大部分实验课都比较糊弄，但是这个实验我觉得是所有实验中唯一值得做的。

*by the way, 计组实验也值得一做，就是那个用 Verilog 写一个五级流水线 CPU，但是那个实验缺乏指导*

对我而言，完成这个实验可以获得以下体会：
- 实践了 C++ 面向对象程序设计
- 动手设计了一个玩具编译器，了解了其执行的全过程，包括可视化AST，输出IR，输出Arm32等
- clangd + vscode + cmake + vscode 自动生成任务和调试，学习通过这个工具链编写 C++ 代码
- 学习使用 Git 做版本管理
- ...

## 0.3 其他

实验原仓库（曾老师的仓库）：https://github.com/NPUCompiler/exp04-minic-expr
